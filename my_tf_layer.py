...
def conv2d_layer(x, kernel_size,out_channel, device = '/gpu:0',stride = 1,name = 'conv2d',padding='SAME'):
    '''
    ...
    with tf.variable_scope(name):
        with tf.device(device):
            ...
            ret = tf.nn.relu(tf.nn.conv2d(x, W, strides=[1, stride, stride, 1], padding=padding) +b , name = 'conv_output')
...
def deconv_layer(x, W_shape, b_shape, device = '/gpu:0', name='deconv' , padding='SAME'):
    '''
    ...
    with tf.variable_scope(name):
        with tf.device(device):
            ...
            return tf.nn.relu(tf.nn.conv2d_transpose(x, W, out_shape, [1, 1, 1, 1], padding=padding) + b,name='deconv_output')


def pool_layer(x, shrink_size = 2, device = '/gpu:0', name='pool',padding = 'SAME'):
    '''
    ...
    with tf.variable_scope(name):
        with tf.device(device):
            ret = tf.nn.max_pool(x, 
                ...
                name = 'max_pool_output',
                padding = padding)
            print 'layer: {}  size: {}'.format( name, ret.get_shape())
            return ret


def up_pool2x2_layer(mat_var, device = '/gpu:0',name='up_sample',padding = 'SAME'):
    '''
    my unpool layer
    expand 
    NHWC
    ...
    with tf.variable_scope(name):
        with tf.device(device):
            ...
            # put back
            mat_back = tf.reshape(mat_colum_ext,[batch,2*height,2*width,channel],name = 'upsample')  
            print 'layer: {}  size: {}'.format( name, mat_back.get_shape())
            return mat_back

def up_pool_deconv_layer(x,up_mag,out_channel = None, device = '/gpu:0', name = 'upsample',padding = "SAME"):
    '''
    ...
    with tf.variable_scope(name):
        with tf.device(device):
            ...
            '''
            Output shape None batch for placeholder
            https://groups.google.com/a/tensorflow.org/forum/#!topic/discuss/vf8eH9YMwVA 
            #out_shape = [-1,up_mag*height,up_mag*width,out_channel]
            bug: feed data
            '''
            out_shape = [shape[0],up_mag*height,up_mag*width,out_channel]
            stride = [1,up_mag,up_mag,1]
            deconv = tf.nn.conv2d_transpose(x,W,out_shape,stride,name = 'conv_transpose',padding = padding) + b #,name = 'upsample')
            print 'layer: {}  size: {}'.format(name,tf.shape(deconv))
            return deconv
...
def read_label(label_name,rotate = 0,size = 512):
    batch_ys = read_img_raw(label_name,size = size,rotate = rotate,type=np.uint8)
